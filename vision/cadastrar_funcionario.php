...
require_once 'logged.php';
	...
	if($_POST)
	{
		if($_POST["empresa"]=="")
		{
			echo"<script>alert('Selecione uma empresa para o funcionário')</script>";
			$erro++;
		}
		if($_POST["perfil"]=="")
		{
		...
		if($erro==0)
		{
			switch ($oper)
			{
                ...
                case "A":
                    //Alterar no BD	
					$funcionario = new funcionario($id, $_POST["perfil"], $_POST["empresa"], $_POST["status"], $_POST["nome"], $_POST["email"], $_POST["senha"]);
                    $funcionarioDAO = new funcionarioDAO();
...
<html lang="pt-br">
    <head>		
        <meta name="description" content="Cadastro de Funcionários"/>
        ...
        <header>
			...
			<div class="conteudo-form">										
						<form action="#" method="POST" id="cadastro_funcionario" class="form-funcionario">
							...
							<div class="row">
							<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">	                                    <label for="empresa">Empresa:</label>	
                                    <select name="empresa" class="form-control" id="empresa">                                        
										<?php
                                            ...
                                            $ret = $empresaDAO->buscarTodas();
                                            foreach($ret as $dado)
											{ 
												if($oper == "A" && $dado->id_empresa == $id_empresa){
													echo"<option value='{$dado->id_empresa}' selected>{$dado->descritivo}</option>";
												}else{
													echo"<option value='{$dado->id_empresa}'>{$dado->descritivo}</option>";
												}
											}
                                        ?>	
                                    </select>
                                </div>
								<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">	                                    <label for="perfil">Perfil:</label>	
							...
							<a href="cadastrar_funcionario.php" ><input type="submit" value="Cadastrar" role="button" class="btn btn-dark enviar" id="salvar"/></a>
							<a href="listar_funcionario.php" ><input type="cancel" value="Cancelar" role="button" class="btn btn-dark enviar" /></a>							
							</div>	
						</div>
					</form>						
				</div>			
